...
my $file2 = "/home/production/public/wheat90k_map.txt";
my $file3 = "/home/production/public/42K-snp-positions.csv";
my $file4 = "/home/production/public/GMS90K-WheatCAP-protocol-2205.vcf";
...
my %variantList;
$q = "SELECT species_name, marker_name, chrom, pos, variant_name, ref, alt from materialized_markerview where reference_genome_name = 'RefSeq_v1'";
$sth = $dbh->prepare($q);
$sth->execute();
...
while ($line = <IN>) {
    ...
    if ($posList{$name}) {
        #print "$name $posList{$name} $lineA[3]\n";
    } else {
        if ($lineA[3] =~ /(\d+)/) {
          $posList{$name} = $1;
        } else {
